...
const QuestController = {
  ...
  createQuest: async (req, res) => {
    try {
      ...
      questData.pointsAchievable =
        parseInt(questData.pointsAchievable, 10) || 0;
      ...
      return res
        .status(201)
        .json({ message: "Quest created successfully", quest: data[0] });
    } catch (err) {
  ...
  add: async (req, res) => {
    try {
      ...
      const { data: uq, error: uqErr } = await QuestModel.addForUser(
        questPayload,
        sb
      );
      ...
      const { data: questRow, error: qErr } = await QuestModel.getQuestById(
        questId,
        sb
      );
      ...
      if (questRow && questRow.huntId) {
        ...
        const { data: huntRow, error: hErr } = await QuestModel.getHuntById(
          questRow.huntId,
          sb
        );
  ...
  complete: async (req, res) => {
    try {
      ...
      if (!userQuestId)
        return res.status(400).json({ message: "Invalid userQuest id" });

      // read the row (RLS ensures user owns it)
      const { data: uq, error: uqErr } = await QuestModel.getUserQuestById(
        userQuestId,
        sb
      );
      if (uqErr) return res.status(400).json({ message: uqErr.message });
      ...
      // verify quest + points
      const { data: quest, error: qErr } = await QuestModel.getQuestById(
        uq.questId,
        sb
      );
      if (qErr) return res.status(400).json({ message: qErr.message });
      if (!quest)
        return res
          .status(404)
          .json({ message: `Quest ${uq.questId} not found` });

      const points = parseInt(quest.pointsAchievable, 10) || 0;

      // mark complete (RLS still in effect)
      const { data: upd, error: updErr } = await QuestModel.setCompleteById(
        userQuestId,
        sb
      );
  ...
  updateQuest: async (req, res) => {
    try {
      ...
      if (isNaN(questId))
        return res.status(400).json({ message: "Quest ID is invalid" });

      let questData = req.body;

      // Safely convert optional numeric fields
      if (
        questData.locationId !== undefined &&
      ...
      if (
        questData.pointsAchievable !== undefined &&
        questData.pointsAchievable !== null &&
        questData.pointsAchievable !== ""
      ) {
